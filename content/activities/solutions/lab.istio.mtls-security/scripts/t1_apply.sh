#!/bin/bash
# Reference solution for lab.istio.mtls-security (t1_apply.sh).
#
# BLOCKED (local, Rs.0 environment): fx.istio-installed.v1 has no handler; K8S_ASSERT targets PeerAuthentication (istio CRD), unsupported
#
...
echo "BLOCKED: fx.istio-installed.v1 has no handler; K8S_ASSERT targets PeerAuthentication (istio CRD), unsupported" >&2
exit 1
