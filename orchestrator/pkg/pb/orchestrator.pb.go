...
type ProvisionRequest struct {
	...
	// PLAN.md M1.4/M1.14: doc §3.2/§7.3's health_gate array (contracts/
	// activity_spec.schema.json), JSON-encoded (matching InjectFaultRequest.params'
	// "wire type is a plain string, richer shape parsed by the receiver"
	// precedent, rather than a new typed message per check kind) --
	// checks that must pass, in order, before any fault is applied (or,
	// absent PRODUCTION_SIM faults, before the environment is considered
	// READY). Empty string means "no richer health_gate declared,"
	// matching every GUIDED_LAB activity today (pod-Ready remains the
	// whole gate for those, exactly as before this field existed).
	HealthGateJson string `protobuf:"bytes,11,opt,name=health_gate_json,json=healthGateJson,proto3" json:"health_gate_json,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
func (x *ProvisionRequest) GetHealthGateJson() string {
	if x != nil {
		return x.HealthGateJson
	}
	return ""
}
...
type ConnectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	EnvironmentId string                 `protobuf:"bytes,1,opt,name=environment_id,json=environmentId,proto3" json:"environment_id,omitempty"`
	// Ownership check (same pattern as InjectFaultRequest.attempt_id below):
	// the RPC handler verifies this matches env.environment.attempt_id for
	// environment_id before minting a session token, returning
	// PermissionDenied on mismatch. Required (empty is rejected).
...
func (x *ConnectRequest) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
...
type MintCredentialsRequest struct {
	...
	// Ownership check (same pattern as InjectFaultRequest.attempt_id below):
	// the RPC handler verifies this matches env.environment.attempt_id for
	// environment_id before minting a credential, returning PermissionDenied
	// on mismatch. Required (empty is rejected).
...
func (x *MintCredentialsRequest) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
...
type DestroyRequest struct {
	...
	// Ownership check (same pattern as InjectFaultRequest.attempt_id below):
	// the RPC handler verifies this matches env.environment.attempt_id for
	// environment_id before tearing it down, returning PermissionDenied on
	// mismatch. Required (empty is rejected) EXCEPT when environment_id
	// doesn't exist at all -- that case returns AlreadyDestroyed=true before
	// any ownership check runs, since there is no owner row left to check
	// against and a double-destroy of an already-gone environment is
	// success, not a caller error (see DestroyResponse's own comment).
...
func (x *DestroyRequest) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
...
type InjectFaultRequest struct {
	...
	// PHASE2_CLOSEOUT.md's flagged access-control gap: InjectFault
	// previously had no way to verify the caller's attempt actually owns
	// environment_id -- any caller holding the service-level shared secret
	// (internal/orchestrator/auth.go) could target ANY environment,
	// including another learner's. The RPC handler now verifies this
	// matches env.environment.attempt_id for environment_id before
	// applying anything, returning PermissionDenied on mismatch. Required
	// (empty is rejected, same InvalidArgument stance as Provision's own
	// attempt_id field) -- this is exactly the ownership check that field
...
func (x *InjectFaultRequest) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
...
type ExecValidatorRequest struct {
	...
	// Ownership check (same pattern as InjectFaultRequest.attempt_id below):
	// the RPC handler verifies this matches env.environment.attempt_id for
	// environment_id before executing the validator, returning
	// PermissionDenied on mismatch. Required (empty is rejected).
...
func (x *ExecValidatorRequest) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
...
type ExecShellRequest struct {
	...
	// Ownership check (same pattern as InjectFaultRequest.attempt_id below):
	// the RPC handler verifies this matches env.environment.attempt_id for
	// environment_id before running the command, returning PermissionDenied
	// on mismatch. Required (empty is rejected).
	AttemptId     string `protobuf:"bytes,4,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
...
func (x *ExecShellRequest) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
...
const file_orchestrator_proto_rawDesc = "" +
	...
	"\aversion\x18\x02 \x01(\tR\aversion\"\x9c\x04\n" +
	...
	"\x10health_gate_json\x18\v \x01(\tR\x0ehealthGateJson\"\xd4\x01\n" +
	...
	"\tendpoints\x18\x03 \x01(\v2/.practiceengine.orchestrator.v1.ConnectResponseR\tendpoints\"V\n" +
	"\x0eConnectRequest\x12%\n" +
	"\x0eenvironment_id\x18\x01 \x01(\tR\renvironmentId\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x02 \x01(\tR\tattemptId\"\xce\x01\n" +
	...
	"\n" +
	"attempt_id\x18\x04 \x01(\tR\tattemptId\"_\n" +
	...
	"\n" +
	"attempt_id\x18\x03 \x01(\tR\tattemptId\">\n" +
	"\x0fDestroyResponse\x12+\n" +
	"\x11already_destroyed\x18\x01 \x01(\bR\x10alreadyDestroyed\"\xad\x02\n" +
	...
	"\x06params\x18\x04 \x03(\v2>.practiceengine.orchestrator.v1.InjectFaultRequest.ParamsEntryR\x06params\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x05 \x01(\tR\tattemptId\x1a9\n" +
	...
	"\x13regressed_resources\x18\x02 \x03(\tR\x12regressedResources\"\xf8\x01\n" +
	...
	"\n" +
	"attempt_id\x18\a \x01(\tR\tattemptId\"\x9a\x01\n" +
	...
	"\n" +
	"attempt_id\x18\x04 \x01(\tR\tattemptId\"\xa6\x01\n" +
	...
	"\tExecShell\x120.practiceengine.orchestrator.v1.ExecShellRequest\x1a1.practiceengine.orchestrator.v1.ExecShellResponseB\x9f\x02\n" +
	"\"com.practiceengine.orchestrator.v1B\x11OrchestratorProtoP\x01ZLgithub.com/tanyasawarn/skillfyme-hands-on/orchestrator/pkg/pb;orchestratorv1\xa2\x02\x03POX\xaa\x02\x1ePracticeengine.Orchestrator.V1\xca\x02\x1ePracticeengine\\Orchestrator\\V1\xe2\x02*Practiceengine\\Orchestrator\\V1\\GPBMetadata\xea\x02 Practiceengine::Orchestrator::V1b\x06proto3"
